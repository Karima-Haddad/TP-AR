    ...
    <div className="app-shell">
      ...
      <div className="layout">
        <div className="main">
          ...
          <div className="algo-header">
            ...
            <div className="header-meta">
              ...
              <span className="meta-chip">
                {algo.id === 'sequencer' ? 'séquenceur central'
                  : algo.id === 'fifo' ? 'canal FIFO'
                  : 'horloge vectorielle'}
              </span>
            </div>
          </div>

          {/* CONTENT : canvas-col + rpanel */}
          ...
          {/* BOTTOM BAR — toute la largeur */}
          <div className="bottom-bar">
            <div className="step-info">Étape {stepIndex + 1} / {steps.length}</div>
            <div className="progress-track">
              <div className="progress-fill" style={{ width: `${progress}%` }} />
            </div>
            <div className="controls">
              {/* PLAY / PAUSE */}
              <button className="ctrl-btn primary" onClick={() => setPlaying(p => !p)} title={playing ? 'Pause' : 'Lecture automatique'} disabled={stepIndex >= steps.length - 1 && !playing}>
                {playing ? (
              ...
              <button className="ctrl-btn" onClick={prev} title="Précédent" disabled={stepIndex === 0 || playing}>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6">
                  <path d="M10 4L5 8l5 4"/>
                  <rect x="4" y="4" width="1.5" height="8" rx="0.5" fill="currentColor" stroke="none"/>
                </svg>
              </button>
              <button className="ctrl-btn" onClick={next} title="Suivant" disabled={stepIndex >= steps.length - 1 || playing}>
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6">
                  <path d="M6 4l5 4-5 4"/>
                  <rect x="10.5" y="4" width="1.5" height="8" rx="0.5" fill="currentColor" stroke="none"/>
                </svg>
              </button>
              <button className="ctrl-btn" onClick={reset} title="Réinitialiser">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.6">
